...
local game = Game()
local version = ": 1.0b" --added by me (pedro), for making updating version number easier
        ...
        mod:AnyPlayerDo(function(player)
            if player:GetPlayerType() == SimType then
                ...
                if game.Difficulty == 2 and game:GetLevel().GreedModeWave == 10 then
                    Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(80, 160), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(80, 800), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(800, 300), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(800, 2000), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(game:GetRoom():GetCenterPos()),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                elseif game.Difficulty == 3 and game:GetLevel().GreedModeWave == 11 then
                    Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(80, 160), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(0, 800), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(800, 300), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(800, 2000), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                        Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(game:GetRoom():GetCenterPos()),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                end
    ...
    if room:GetType() == RoomType.ROOM_BOSS then
        mod:AnyPlayerDo(function(player)
            if player:GetPlayerType() == SimType then
                ...
                Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(80, 160), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(80, 400), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(560, 160), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(Vector(560, 400), 0, true),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
                Isaac.Spawn(5,axeEntity,1,game:GetRoom():FindFreePickupSpawnPosition(game:GetRoom():GetCenterPos()),Vector(playerRNG:RandomFloat() - .5, playerRNG:RandomFloat() - .5) * Vector(2, 2),nil)
            end
        end)
    end 
...
mod:AddCallback(ModCallbacks.MC_EVALUATE_CACHE,function (_,Player,Cache)
    ...
    if Cache == CacheFlag.CACHE_FIREDELAY then
        ...
        local newTears = math.max(math.min(.1, currentTears), currentTears + Data.MaxFireDelay)
...
function mod:saveData()
    ...
    saveTable.Repm_CHAPIData = mod.Repm_CHAPIData
    saveTable.MenuData = mod.MenuData
...
function mod:loadData(isSave)
    if mod:HasData() then
        ...
        mod.Repm_CHAPIData = perData.Repm_CHAPIData
    end
...
function mod:tryOpenDoor_Fro_Polaroid(player)
    ...
    if (game:GetLevel():GetStage() == 6 or (game:GetLevel():GetStage() == 5 and Game():GetLevel():GetCurses() & LevelCurse.CURSE_OF_LABYRINTH ~= 0)) and game:GetLevel():GetStageType() <= 2 
...
--[[if EID then
    ...
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_LIKE, "When Isaac play Like animation add stats.", "Like" )
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_SALAMI, "Gives +0.50 to speed, damage, range, luck and +0.27 tears.", "Salami" )
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_BOOK_OF_TAILS, "Guaranteed to create a crawlspace `Im error` room #Lowers the chance of the devil and angel deal.", "book of tails" )
    ...
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_PRO_KAMIKAZE, "Releases fire, depending on the number of enemies in the room.", "PROkamikadze" )
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_LOST_SHROOM, "Gives 1 soul heart# deals poison damage to a random enemy in the room.", "Lost shroom" )
    ...
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_STRAWBERRY_MILK, "Changes tears to pink,which leaves pink creep that petrifies enemies# Bosses get the slowness effect.", "strawberry milk" )
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_HOLY_SHELL, "On use, releases 4 holy beams in the + direction.", "Holy shell" )
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_LEAKY_BUCKET, "Spawns a puddle under isaac at random, which petrifies enemies.", "Leaky Bucket" )
   ...
   -- EID:addCollectible(mod.RepmTypes.COLLECTIBLE_VACUUM, "Gives 5,25 range#Have a chance to shoot a boomerang tear that deals damage to enemies.", "vacuum" )
    ...
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_PINK_STRAW, "On use, inflicts random debuff on enemies in the room.", "Pink straw" )
    ...
    EID:addCollectible(mod.RepmTypes.COLLECTIBLE_DILIRIUM_EYE, "Have a chance to shoot 5 tears#The chance doesnt relly on luck.", "Dilirium eye" )
    EID:addCollectible(mod.RepmTypes.Collectible_HOLY_OTMICHKA, "Have a chance to change any chest into holy chest.", "Holy master key" )
    EID:addCollectible(mod.RepmTypes.Collectible_FLOWER_TEA, "Gives 0.60 damage, 0.50 range#Lowers shot speed by 0.20.", "Flower tea" )
    ...
    EID:addCollectible(mod.RepmTypes.Collectible_SANDWICH, "Gives 0.50 damage and 0.09 tears.", "sandwich" )
    ...
    EID:addCollectible(mod.RepmTypes.Collectible_HOW_TO_DIG, "Isaac is buried underground, monsters cannot attack him.", "How To Dig" )
    EID:addCollectible(mod.RepmTypes.Collectible_BATTERED_LIGHTER, "Lights fires.", "Battered Lighter" )
...
--ru
    ...
    EID:addCard(iceCard, "Using it in most rooms slowly begins to freeze all enemies. #Non-boss enemies in the room gradually turn blue before freezing completely#Use this item in the Mom fight for a different effect...", "Icicle" )
    local iceHud = Sprite()
