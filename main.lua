...
function mod:MeatySound(entityTear, collider, low)
	if collider:IsActiveEnemy() and collider:IsVulnerableEnemy() then
		...
		collider:TakeDamage(entityTear.CollisionDamage, _, EntityRef(entityTear.Parent), 0)
		Isaac.GetItemConfig():GetCollectible(mod.RepmTypes.COLLECTIBLE_AXE_ACTIVE).MaxCharges = Isaac.GetItemConfig()
			:GetCollectible(mod.RepmTypes.COLLECTIBLE_AXE_ACTIVE).MaxCharges + 50
	else
		return true
	end
end
...
function mod:onFireAxe(player)
	if player:GetData().repM_fireAxe == true then
		...
		for y = 1, multiples, 1 do
			...
			if player:HasCollectible(CollectibleType.COLLECTIBLE_BIRTHRIGHT) then
				tear:AddTearFlags(TearFlag.TEAR_HP_DROP)
			end
...
function mod:NewRoomAXE()
	...
	then
		mod:AnyPlayerDo(function(player)
			if player:GetPlayerType() == SimType and room:IsFirstVisit() then
				local playerRNG = player:GetDropRNG()
...
function Holyshell:onUpdate(player)
	...
	--заряд
	if
		player:GetActiveItem() == mod.RepmTypes.COLLECTIBLE_HOLY_SHELL
		or player:GetActiveItem() == mod.RepmTypes.COLLECTIBLE_UNHOLY_SHELL
	then
...
function Holyshell:onHolyshell1(_, rng, player)
...
function Holyshell:onHolyshell2(_, rng, player)
                    ...
                    for i = -2, 2 do
                        if i ~= 0 then
                            local ShootDirection = Vector(
                                -math.cos(math.rad(15 * math.abs(i))) * player.ShotSpeed * 10,
                                -math.sin(math.rad(15 * math.abs(i))) * player.ShotSpeed * 10 * i / math.abs(i)
                            )
...
if EID then
	...
	EID:addCollectible(
		...
		"Guaranteed to create a crawlspace `Im error` room #Lowers the chance of the devil and angel deal.",
		"book of tails"
	)
	...
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_PRO_KAMIKAZE,
		"Releases fire, depending on the number of enemies in the room.",
		"PROkamikadze"
	)
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_LOST_SHROOM,
		"Gives 1 soul heart# deals poison damage to a random enemy in the room.",
		"Lost shroom"
	)
	EID:addCollectible(
	...
	EID:addCollectible(
		...
		"Changes tears to pink,which leaves pink creep that petrifies enemies# Bosses get the slowness effect.",
		"strawberry milk"
	)
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_HOLY_SHELL,
		"On use, releases 4 holy beams in the + direction.",
		"Holy shell"
	)
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_LEAKY_BUCKET,
		"Spawns a puddle under isaac at random, which petrifies enemies.",
		"Leaky Bucket"
	)
	EID:addCollectible(
	...
	EID:addCollectible(
		...
		"On use, inflicts random debuff on enemies in the room.",
		"Pink straw"
	)
	EID:addCollectible(
	...
	EID:addCollectible(
		...
		"Have a chance to shoot 5 tears#The chance doesnt relly on luck.",
		"Dilirium eye"
	)
	EID:addCollectible(
		mod.RepmTypes.Collectible_HOLY_OTMICHKA,
		"Have a chance to change any chest into holy chest.",
		"Holy master key"
	)
	EID:addCollectible(
	...
	EID:addCollectible(
		...
		"When fully charged, Isaac begins to scream, stunning enemies.",
		"Siren Horns"
	)
	EID:addCollectible(
		mod.RepmTypes.Collectible_HOW_TO_DIG,
		"Isaac is buried underground, monsters cannot attack him.",
		"How To Dig"
	)
	...
	EID:addCollectible(
		...
		"Когда Айзек проигрывает 'Like' анимацию - прибавка к характеристикам.",
		"Лайк",
		"ru"
	)
	...
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_BOOK_OF_TAILS,
		"Гарантировано создаёт подпольную команту «Я ошибка». # Снижает вероятность сделки дьявола и ангела.",
		"Книга сказок",
		"ru"
	)
	...
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_LOST_SHROOM,
		"Дает 1 сердце души # накладывает яд случайному врагу в комнате.",
		"Потерянный гриб",
		"ru"
	)
	EID:addCollectible(
	...
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_STRAWBERRY_MILK,
		"Создает лужи под врагами, если враг наступит на лужу, то окаменеет",
		"Клубничное молоко",
		"ru"
	)
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_HOLY_SHELL,
		"При использовании выпускает 4 святых луча как: +.",
		"Святая оболочка",
		"ru"
	)
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_LEAKY_BUCKET,
		"Иногда создает лужу под Айзеком, которая окаменевает врагов.",
		"Дырявое ведро",
		"ru"
	)
	EID:addCollectible(
	...
	EID:addCollectible(
		mod.RepmTypes.COLLECTIBLE_DILIRIUM_EYE,
		"Есть шанс выстрелить 5 слезами #Шанс не зависит от удачи.",
		"Глаз сумашествия",
		"ru"
	)
	EID:addCollectible(
		mod.RepmTypes.Collectible_HOLY_OTMICHKA,
		"Есть шанс превратить любой сундук в священный сундук.",
		"Святая отмычка",
		"ru"
	)
	EID:addCollectible(
		mod.RepmTypes.Collectible_FLOWER_TEA,
		"Дает 0,60 урона, дальность 0,50 # Снижает скорострельность на 0,20.",
		"Цветочный чай",
		"ru"
	)
	EID:addCollectible(
	...
	EID:addCollectible(
		mod.RepmTypes.Collectible_SIREN_HORNS,
		"При полной зарядке Айзек начинает кричать, оглушая врагов.",
		"Рога сирены",
		"ru"
	)
	EID:addCollectible(
		mod.RepmTypes.Collectible_HOW_TO_DIG,
		"Исаакзакапывается под землю #Монстры не могут на него напасть.",
		"Как копать",
		"ru"
	)
	EID:addCollectible(
	...
	EID:addCard(
		iceCard,
		"Using it in most rooms slowly begins to freeze all enemies. #Non-boss enemies in the room gradually turn blue before freezing completely#Use this item in the Mom fight for a different effect...",
		"Icicle"
	)
	local iceHud = Sprite()
